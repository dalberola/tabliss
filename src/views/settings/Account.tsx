...
// The login/register/logout/delete copy is shared with the auth widget.
import { messages } from "../../plugins/widgets/messages";
import "../../plugins/widgets/auth/AuthSettings.sass";
import { messages as settingsMessages } from "./messages";

/**
 * Global "Account Sync" settings section: sign in / register / log out / delete.
 * Rendered once in the Settings panel (web only — sync relies on the
 * cross-subdomain cookie + CORS path), so there is a single login at all times.
 * The dashboard tile is a separate optional widget.
 */
const Account: FC = () => {
  const intl = useIntl();
  const {
  ...
  const heading = (
    <h2>
      <FormattedMessage {...settingsMessages.accountSync} />
    </h2>
  );
